...
# client.publish( 'esp32', payload='status', qos=0, retain=False )
...
def background_scheduler():
	...
	client.publish( 'esp32', payload='status', qos=0, retain=True )
	client.loop(1)		#blocks for 100ms
	print('Start Loop')
	while True:
		s = datetime.datetime.now().second
		...
		# Not using LoRa, using Bluetooth, but may be movin to MQTT
		# patrol()
...
# def populate(line):
#	Notify only when door is open.
#	De couple getting the door so can use mqtt.
