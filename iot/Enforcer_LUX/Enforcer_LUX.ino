...
int PIR = 27;   // TTGO T-Display Works
int fontsize = 1;     //  ttgo
...
//    Enforcer
const char* client_name           = "sensors_TTGO_Enforcer";
//const char* client_name           = "Office_sensors_M5Stack";
const char* publish_topic         = "enforcer";
const char* publish_topic_lux     = "enforcer/lux";

const char* subscription_terminal = "command/enforcer/terminal";
const char* subscription_sensor   = "command/enforcer";
...
void read_sensors()
{
  ...
  if ( is_espTTGO )
  {
      ...
      {
        interval -= 1000;
        t.lcd_display( 10, 60, fontsize, 1000, String( interval ) );
        t.fill_screen( TFT_BLACK );
      }
  ...
  if( motion )
  {
    ...
    last_time = millis();
    t.lcd_display( 10, 30, fontsize, 100, String( last_time ) );
    motion_count++;

    String mt = "Motion=" + String( motion_count );
    t.lcd_display( 10, 60, fontsize, 0, mt );

    if ( motion_count >= motion_threshold )
    {
        publish_message( publish_topic, String( motion_count ) );
    }
  }
